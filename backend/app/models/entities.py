...
class AiEvaluation(TimestampMixin, Base):
    ...
    analysis_record_id: Mapped[int] = mapped_column(
        ForeignKey("ai_analysis_records.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    metric_name: Mapped[str] = mapped_column(String(64), nullable=False)
    expected_value: Mapped[dict[str, Any] | None] = mapped_column(JSONB)
    actual_value: Mapped[dict[str, Any] | None] = mapped_column(JSONB)
    score: Mapped[Decimal] = mapped_column(Numeric(6, 5), nullable=False)

    analysis_record: Mapped["AIAnalysisRecord"] = relationship(
