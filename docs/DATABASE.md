...
数据库模型当前共 13 张表。全部表包含 `id`、`created_at` 和 `updated_at`；除租户根表 `companies` 外，其余表均包含 `company_id`，用于租户隔离和未来扩展。
...
- `ai_analysis_records`：AI 分析过程和结果记录，包含结构化 JSON、置信度和执行状态
- `ai_evaluations`：AI 质量评估明细，保存指标期望值、实际值和得分
